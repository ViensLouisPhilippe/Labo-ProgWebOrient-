...
export class AppComponent {
  ...
  async getAnimals() : Promise<void>{
	// A
  let x = await lastValueFrom(this.http.get<Animal[]>("https://localhost:7050/api/Animals/GetAnimal"));
  console.log(x);
  this.animals = x;
  }
  ...
  async postAnimal() : Promise<void>{
    ...
    let x = await lastValueFrom(this.http.post<Animal>("https://localhost:7050/api/Animals/PostAnimal", newAnimal));
    console.log(x);
  }
  ...
  async getAnimal() : Promise<void>{
    // I
    let x = await lastValueFrom(this.http.get<Animal>("https://localhost:7050/api/Animals/GetAnimal/" + this.id));
  console.log(x);
  this.animal = x;
  }
  ...
  async putAnimal() : Promise<void>{
    ...
    if(this.id != undefined)
    {
      ...
      let x = await lastValueFrom(this.http.put<Animal>("https://localhost:7050/api/Animals/PutAnimal/" + this.id, updatedAnimal));
      console.log(x);
    }
  }
  ...
  async deleteAnimal() : Promise<void>{
    // A
    let x = await lastValueFrom(this.http.post<Animal>("https://localhost:7050/api/destroy/" + this.id, null));
    console.log(x);
  }
